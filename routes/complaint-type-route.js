...
router.get('/',async(req,res)=>{
    try{    
    const details=await db.ComplaintType.findAll().catch((err)=>{
...
router.get('/:id',async(req,res)=>{
    try{   
     const {id} =req.params;  
     const details=await db.ComplaintType.findOne({
        where:{
            id:id
        }         
    }).catch((err)=>{
...
router.post('/',async(req,res)=>{
    try{
        const {name}=req.body; 
        const details=await db.ComplaintType.create({name});
        if(details)
 ...
 router.put('/:id',async(req,res)=>{
    try{ 
        const {id}=req.params;
        const {name}=req.body; 
        const details=await db.ComplaintType.update({
            name
        },{where:{id}}).catch((err)=>{
 ...
 router.delete('/:id',async(req,res)=>{
    try{
        const {id}=req.params; 
        const details=await db.ComplaintType.destroy({where:{id}}).catch((err)=>{

...
router.get('/',async(req,res)=>{
    try{    
    const details=await db.Route.findAll().catch((err)=>{
...
router.get('/company/:id',async(req,res)=>{
    try{   
     const {id} =req.params;  
     const details=await db.Route.findAll({
        where:{
            companyId:id
        }         
    }) ;
...
router.get('/:id',async(req,res)=>{
    try{   
     const {id} =req.params;  
     const details=await db.Route.findOne({
        where:{
            id:id
        }         
    }) ;
...
router.post('/',async(req,res)=>{
    try{
        ...
        const details=await db.Route.create({name,code,description,companyId});
        if(details)
 ...
 router.put('/:id',async(req,res)=>{
    try{ 
        const {id}=req.params;
        const {name,code,description,companyId}=req.body; 
        const details=await db.Route.update({
 ...
 router.delete('/:id',async(req,res)=>{
    try{
        const {id}=req.params; 
        const details=await db.Route.destroy({where:{id}}).catch((err)=>{

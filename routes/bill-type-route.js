...
router.get('/',async(req,res)=>{
    try{    
    const details=await db.BillType.findAll().catch((err)=>{
...
router.get('/:id',async(req,res)=>{
    try{   
     const {id} =req.params;  
     const details=await db.BillType.findOne({
        where:{
            id:id
        }         
    }).catch((err)=>{
...
router.post('/',async(req,res)=>{
    try{
        const {name}=req.body; 
        const details=await db.BillType.create({name});
        if(details)
 ...
 router.put('/:id',async(req,res)=>{
    try{ 
        const {id}=req.params;
        const {name}=req.body; 
        const details=await db.BillType.update({
            name
        },{where:{id}}).catch((err)=>{
 ...
 router.delete('/:id',async(req,res)=>{
    try{
        const {id}=req.params; 
        const details=await db.BillType.destroy({where:{id}}).catch((err)=>{
